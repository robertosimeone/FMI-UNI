...
const int no_rec_ev_per_set = 20;
const int rec_ev_per_set = 8;
vector<pair<vector<string>,vector<string>>> all_folds;
...
void arrange_kfold_vector(int k){
    ...
    for(int i = 0; i < k; i ++){
        ...
        for(int j = 0;j < rec_ev_per_set+extraRec;j++){
            kfold_vector[i].push_back(recurrence_events.front());
            recurrence_events.pop();
        }
        for(int j = 0;j < no_rec_ev_per_set+extraNoRec;j++){
...
void formKFold(){
//    vector<pair<vector<string>,vector<string>>> all_folds;
    //kfold vector is vector<vector<string>>
    //vector that stores pair of test and train data
    all_folds.resize(10);
    for(int i = 0 ; i < kfold_vector.size();i++){
        int chosen = i;
        all_folds[i].first = kfold_vector[i];
        for(int j = 0; j < kfold_vector.size();j++){
            if(j!=i){
               for(int t = 0 ; t < kfold_vector[j].size();t++){
                   all_folds[i].second.push_back(kfold_vector[j][t]);
               }
            }
        }
    ...
    void calculateTable(){
        for(int i = 0  ; i < current_kfolddata.size();++i){
            ...
            if(forbidden_rows.find(i)==forbidden_rows.end()){
                if(forbidden_cols.find(3)!=forbidden_cols.end() and forbidden_cols.find(6)!=forbidden_cols.end()){
...
public:
    ...
    vector<string> validation_set;
    PostID3(vector<string> test_set){
    ...
    void createValidationSet(){
        State * modifiedDataRoot = root;
        for(int i = 0; i < current_kfolddata.size()/2;i++){
            validation_set.push_back(current_kfolddata[i]);
        }
        for(int i =current_kfolddata.size()/2; i < current_kfolddata.size();i++){
//            cout <<"post pruning"<< current_kfolddata[i] << endl;
        }
    }
    void misslclasification(){
        for(int i= 0 ; i < validation_set.size();i++){

            if(i == 24){
                string c = "10";
            }
            singleClassification(validation_set[i]);
        }
        prune(root);
    }
    void singleClassification(string row){
        State * currentState = root;
        string realLabel = readFromCol(0,row);
        //add decision of current node
        bool flag = false;
        while(true) {
            int column = currentState->bestGainColumn;
            if(currentState->dominant!=realLabel) {
                currentState->missclassifications++;
            }
            if(currentState->isLeafNode){
                return;
            }
            string currentTest = readFromCol(column,row);
            for(int i = 0 ; i < currentState->children.size();i++){
                if(currentState->children[i]->attribute_value == currentTest){
                    currentState=currentState->children[i];
                    break;
                }
                if(i == currentState->children.size()-1 and currentState->children[i]->attribute_value != currentTest){
                    flag = true;
                }
            }
            if(flag) {
    ...
    void prune(State * state){
        if(state->isLeafNode){
            if(state->missclassifications >= state->previous_state->missclassifications){
                state->previous_state->isLeafNode = true;
                delete state;
            }
        }
        for(int i = 0 ; i <state->children.size();i++){
                prune(state->children[i]);
        }
    }
...
//            cout << "leaf node from the buildTree function,wont continue;" << endl << endl;
        ...
        if(!childState->isLeafNode and   childState->forbidden_cols.size() != 9) {
    ...
    string classifyRow(string current_sample){
        ...
        while(!current_node->isLeafNode){
            ...
            for(int i = 0 ; i < current_node->children.size();i++){
                ...
                if(i == current_node->children.size()-1 and current_node->children[i]->attribute_value != test){
                    flag = true;
                }
...
//                cout << i << "vec " <<kfold_vector[test_set][i]<<endl;
        ...
        return (double)right/samples.size();
    }
    ~PostID3(){
        delete root;
    }
...
int main(){
    ...
    readFile(filename);
    arrange_kfold_vector(10);
    formKFold();
//    for(int i = 0 ;i < kfold_vector[0].size();i++){
//        cout << kfold_vector[0][i] << endl;
//    }
//    return 0;
    vector<double> precision_results_pre;
    vector<double> precision_results_post;
    int chosen = -1;
    for(int i = 0 ; i <10;i++){
//        concatCurrentFold(i);
        vector<string> test_data = all_folds[i].first;
        current_kfolddata = all_folds[i].second;
//        if(i == 1){
//
//            for(int i =0 ; i < current_kfolddata.size();i++){
//                cout << i <<" i " << current_kfolddata[i] << endl;
//            }
//            cout << endl << endl;
//            for(int i =0 ; i < current_kfolddata.size();i++){
//                if(current_kfolddata[i] == ""){
//                    cout << i << " " << "empty roll" << endl << endl;
//                }
//            }
//
//        }
...
//        }
        ...
        PreID3 pre_tree = PreID3(test_data);

        PostID3 post_tree = PostID3(test_data);
//        cout << "im not here" << endl;


        precision_results_pre.push_back(pre_tree.calculate_precision());
        precision_results_post.push_back(post_tree.calculate_precision());

    }
    double totalPre = 0;
    double totalPost = 0;
    cout << "---------------------------------------------------------------------------------------------------------" << endl<<endl;
    for(int  i = 0 ; i < precision_results_pre.size();i++){
        totalPre+=precision_results_pre[i];
        cout << "Test :" << i << endl;
        cout <<"Pre pruning tree : "<< precision_results_pre[i] << endl;
    }
    cout << endl << endl;
    cout << "---------------------------------------------------------------------------------------------------------" << endl<<endl;
    for(int  i = 0 ; i < precision_results_pre.size();i++){
        totalPost+=precision_results_post[i];
        cout << "Test :" << i << endl;
        cout << "Post pruning tree : "<< precision_results_post[i] << endl;
    }
    cout << endl << endl;
    cout << "---------------------------------------------------------------------------------------------------------" << endl<<endl;
    cout << "Pre pruning average from 10 runs : "<< (double)totalPre/precision_results_pre.size() <<endl;
    cout << "Post pruning average from 10 runs : "<< (double)totalPost/precision_results_post.size()<<endl;
    cout << "---------------------------------------------------------------------------------------------------------" << endl<<endl;
    return 0;
}